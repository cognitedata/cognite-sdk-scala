  ...
  implicit val instancePropertyTypeEncoder: Encoder[InstancePropertyValue] =
    Encoder.instance[InstancePropertyValue] {
      ...
      case DateList(values) =>
        Json.arr(values = values.map(d => Json.fromString(d.format(InstancePropertyValue.Date.formatter))): _*)
      case TimestampList(values) =>
        Json.arr(values = values.map(d => Json.fromString(d.format(InstancePropertyValue.Timestamp.formatter))): _*)
