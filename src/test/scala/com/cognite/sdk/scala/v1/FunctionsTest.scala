...
class FunctionsTest extends CommonDataModelTestHelper with Matchers with ReadBehaviours {
  ...
  ignore should "create and delete functions" in {
    val createRes = client.functions
      .create(
        Seq(
          FunctionCreate(
  ...
  it should "retrieve function call by id" in {
    val call =
      client.functionCalls(8590831424885479L).retrieveById(3987350585370826L).unsafeRunSync()
  ...
  it should "read function call logs items" in {
    val res =
      client.functionCalls(8590831424885479L).retrieveLogs(3987350585370826L).unsafeRunSync()
    res.items.isEmpty shouldBe true
  }

  it should "retrieve function call response" in {
    val response =
      client.functionCalls(8590831424885479L).retrieveResponse(3987350585370826L).unsafeRunSync()
    response.response shouldBe Some(
      Json.fromFields(
        Seq(("res_int", Json.fromInt(1)), ("res_string", Json.fromString("string response")))
      )
    )
  }
  ...
  it should "call function" in {
    val nonce = client.sessions
      .createWithClientCredentialFlow(
        Items(Seq(SessionCreateWithCredential(credentials.clientId, credentials.clientSecret)))
      )
      .unsafeRunSync()
      .map(_.nonce)
      .head
    val res = client
      .functionCalls(8590831424885479L)
      .callFunction(Json.fromJsonObject(JsonObject.empty), Some(nonce))
      .unsafeRunSync()
  ...
  it should "create and delete function schedules" in {
    val nonce = client.sessions
      ...
      .map(_.nonce)
      .head
    val createRes = client.functionSchedules
      .create(
        Seq(
    ...
    client.functionSchedules
      .read()
      .unsafeRunSync()
      .items
      .find(_.name === "scala-sdk-write-example-1") shouldBe None
  }
}
