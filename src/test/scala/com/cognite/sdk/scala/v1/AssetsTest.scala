...
class AssetsTest extends SdkTestSpec with ReadBehaviours with WritableBehaviors with RetryWhile {
  ...
  (it should behave).like(
    readableWithRetrieve(client.assets, idsThatDoNotExist, supportsMissingAndThrown = true)
  )
  ...
  (it should behave).like(
    writable(
      ...
      Seq(
        AssetCreate(name = "scala-sdk-create-example-1"),
        AssetCreate(name = "scala-sdk-create-example-2")
      ),
      idsThatDoNotExist,
      supportsMissingAndThrown = true
    )
  ...
  it should "support deleting entire asset subtrees recursively by externalId" in {
    ...
    val assetTree = Seq(
      ...
      AssetCreate(
        name = "child",
        externalId = Some(s"$key-recursive-child"),
        parentExternalId = Some(s"$key-recursive-root")
      ),
      AssetCreate(
        name = "grandchild",
        externalId = Some(s"$key-recursive-grandchild"),
        parentExternalId = Some(s"$key-recursive-child")
      )
    )
    ...
    try {
      retryWithExpectedResult[Seq[Asset]](
        client.assets
          .filter(AssetsFilter(externalIdPrefix = Some(s"$key-recursive")))
          .compile
          .toList
          .unsafeRunSync(),
      ...
      client.assets
        .deleteRecursive(Seq(CogniteExternalId(s"$key-recursive-root")), true, true)
        .unsafeRunSync()
      ...
      try
        client.assets
          .delete(createdAssets.map(event => CogniteInternalId(event.id)), ignoreUnknownIds = true)
          .unsafeRunSync()
      catch {
  ...
  it should "support deleting entire asset subtrees recursively by id" in {
    ...
    try {
      ...
      client.assets
        .deleteRecursive(Seq(CogniteInternalId(createdAssets(0).id)), true, true)
        .unsafeRunSync()
  ...
  private def createAssets(externalIdPrefix: String) = {
    ...
    retryWithExpectedResult[Seq[Asset]](
      client.assets
        .filter(AssetsFilter(externalIdPrefix = Some(s"$externalIdPrefix")))
        .compile
        .toList
        .unsafeRunSync(),
  ...
  it should "support deleting by CogniteIds" in {
    ...
    retryWithExpectedResult[Seq[Asset]](
      client.assets
        .filter(AssetsFilter(externalIdPrefix = Some(externalIdPrefix)))
        .compile
        .toList
        .unsafeRunSync(),
      r => r should have size 0
  ...
  it should "raise a conflict error if input of delete contains internalIdand externalId that represent the same row" in {
    ...
    retryWithExpectedResult[Seq[Asset]](
      client.assets
        .filter(AssetsFilter(externalIdPrefix = Some(prefix)))
        .compile
        .toList
        .unsafeRunSync(),
      r => r should have size 0
  ...
  (it should behave).like(
    updatable(
      ...
      (readAssets: Seq[Asset], updatedAssets: Seq[Asset]) => {
        ...
        assert(updatedAssets.zip(readAssets).forall { case (updated, read) =>
          updated.name === s"${read.name}-1"
        })
  ...
  it should "update labels on assets" in {
    ...
    client.labels
      .createItems(
        Items(
          Seq(
            LabelCreate(externalId = externalId1, name = externalId1),
    ...
    val assetToCreate = Seq(
      AssetCreate(
        ...
        name = externalId1,
        labels = Some(Seq(CogniteExternalId(externalId1))),
    ...
    val updatedAssets: Seq[Asset] = client.assets
      .updateByExternalId(
        Map(
          ...
          externalId1 -> AssetUpdate(
            metadata = Some(UpdateMap(add = Map("test2" -> "test2"))),
            labels = Some(
              UpdateArray(
                add = Seq(CogniteExternalId(externalId2)),
                remove = Seq(CogniteExternalId(externalId1))
              )
            )
          ),
          // Set labels to label with externalId=externalId2 on asset2
          externalId2 -> AssetUpdate(
            metadata = Some(SetValue(set = Map("test2" -> "test2"))),
    ...
    updatedAssets.head.metadata.toList.head should contain theSameElementsAs Map(
      "test1" -> "test1",
      "test2" -> "test2"
    )
    ...
    client.assets
      .updateOneByExternalId(externalId1, AssetUpdate(labels = Some(UpdateArray())))
      .unsafeRunSync()
    // Test with empty list on remove, basically add label with externalId3
    val updated = client.assets
      .updateOneByExternalId(
        externalId1,
        AssetUpdate(labels = Some(UpdateArray(add = Seq(CogniteExternalId(externalId3)))))
      )
      .unsafeRunSync()
    updated.labels.toList.head should contain theSameElementsAs Seq(
      CogniteExternalId(externalId2),
  ...
  it should "update metadata on assets with empty map" in {
    ...
    val updatedAssetsWithNone: Seq[Asset] = client.assets
      .updateByExternalId(
        Map(externalId1 -> AssetUpdate(name = Some(SetValue("ML :)")), metadata = None))
      )
      .unsafeRunSync()

    val updatedAssets: Seq[Asset] = client.assets
      .updateByExternalId(Map(externalId1 -> AssetUpdate(metadata = Some(SetValue(set = Map())))))
      .unsafeRunSync()
  ...
  (it should behave).like(
    updatableByExternalId(
      ...
      Map(
        update1ExternalId -> AssetUpdate(name = Some(SetValue("update-1-1"))),
  ...
  it should "support filter" in {
    ...
    val assetSubtreeIdsFilterResult = client.assets
      .filter(
        AssetsFilter(assetSubtreeIds = Some(Seq(CogniteInternalId(3028597755787717L))))
      )
      .compile
      .toList
      .unsafeRunSync()
      ...
      .search(
        AssetsQuery(
          filter = Some(
            AssetsFilter(
              createdTime = Some(
                TimeRange(Some(Instant.ofEpochMilli(0)), Some(Instant.ofEpochMilli(1560756460294L)))
              ),
