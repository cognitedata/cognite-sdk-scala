...
class SequencesTest extends SdkTestSpec with ReadBehaviours with WritableBehaviors with RetryWhile {
    ...
    writable(
      ...
      Seq(
        ...
        Sequence(
          ...
          columns = NonEmptyList.of(
            SequenceColumn(name = Some("col1"), externalId = Some(s"ext1-$eid"), valueType = "LONG")
          )
        )
      ),
      Seq(
    ...
    writableWithExternalId(
      ...
      Seq(
        Sequence(
          ...
          columns = NonEmptyList.of(
            SequenceColumn(name = Some("string-column"), externalId = Some(s"ext2-$eid"))
          )
        ),
        Sequence(
      ...
      Seq(
        SequenceCreate(
          ...
          externalId = Some(shortRandom()),
          columns = NonEmptyList.of(
            SequenceColumnCreate(name = Some("string-column"), externalId = s"string-column-$eid")
          )
        ),
        SequenceCreate(
  ...
  private val sequencesToCreate = Seq(
    Sequence(
      ...
      columns = NonEmptyList.of(
        SequenceColumn(name = Some("string-column"), externalId = Some(s"string-column-$eid"))
      )
    ),
    Sequence(
      name = Some("scala-sdk-write-example-2"),
  ...
  (it should behave).like(
    updatable(
      ...
      (readSequence: Seq[Sequence], updatedSequence: Seq[Sequence]) => {
        ...
        assert(updatedSequence.zip(readSequence).forall { case (updated, read) =>
          updated.name === read.name.map(n => s"${n}-1")
        })
  ...
  (it should behave).like(
    updatableById(
      ...
      Seq(
        ...
        SequenceUpdate(
          name = Some(SetValue("scala-sdk-write-example-2-1")),
          dataSetId = Some(SetValue(testDataSet.id)),
          columns = Some(
            SequenceColumnsUpdate(
              add = Some(
                Seq(
                  SequenceColumnCreate(
                    name = Some("new-column-new-starts"),
                    externalId = s"${sequencesToCreate(2).columns.head.externalId.getOrElse("")}-2"
                  )
                )
              ),
              remove = Some(
                Seq(CogniteExternalId(sequencesToCreate(2).columns.head.externalId.getOrElse("")))
              )
            )
          )
        ),
        SequenceUpdate(
          name = Some(SetValue("scala-sdk-write-example-3-1")),
      ...
      (readSequences: Seq[Sequence], updatedSequences: Seq[Sequence]) => {
        ...
        secondSequenceFirstColumn.flatMap(_.externalId) shouldBe sequencesToCreate
          .lift(2)
          .flatMap(_.columns.head.externalId.map(ext => s"$ext-2"))
    ...
    updatableByExternalId(
      ...
      Seq(
        ...
        Sequence(
          ...
          externalId = Some(s"update-2-externalId-$eid")
        )
      ),
      Map(
        s"update-1-externalId-$eid" -> SequenceUpdate(externalId =
          Some(SetValue(s"update-1-externalId-$eid-1"))
        ),
        s"update-2-externalId-$eid" -> SequenceUpdate(externalId =
          Some(SetValue(s"update-2-externalId-$eid-1"))
        )
      ),
      (readSequences: Seq[Sequence], updatedSequences: Seq[Sequence]) => {
        assert(readSequences.size == updatedSequences.size)
  ...
  it should "support search" in {
      ...
      .search(
        SequenceQuery(
          filter = Some(
            SequenceFilter(
              createdTime = Some(
                TimeRange(Some(Instant.ofEpochMilli(0)), Some(Instant.ofEpochMilli(1568975105000L)))
              )
            )
          )
        )
    ...
    val createdTimeSearchResults2 = client.sequences
      .search(
        SequenceQuery(
          filter = Some(
            SequenceFilter(
              createdTime = Some(
                TimeRange(
                  ...
                  Some(Instant.ofEpochMilli(1568979128000L))
                )
              )
            )
          )
    ...
    val externalIdPrefixSearchResults = client.sequences
      .search(
        SequenceQuery(
          filter = Some(
            SequenceFilter(
              ...
              createdTime = Some(
                TimeRange(Some(Instant.ofEpochMilli(0)), Some(Instant.ofEpochMilli(1568980123000L)))
              )
            )
          )
        )
    ...
    val descriptionSearchResults = client.sequences
      .search(
        SequenceQuery(
          filter = Some(
            SequenceFilter(
    ...
    val limitDescriptionSearchResults = client.sequences
      .search(
        SequenceQuery(
          limit = 1,
          filter = Some(
