...
class DataPointsTest extends SdkTestSpec with DataPointsResourceBehaviors {
  ...
  it should "be possible to correctly query aggregate values" in {
    val aggregates = client.dataPoints
      .queryAggregatesById(
        54577852743225L,
    ...
    (averages.map(_.timestamp).tail should contain)
      .theSameElementsInOrderAs(stepInterpolations.map(_.timestamp))
    averages.last.value should equal(2.7051279586700723 +- delta)

    val limit = 10
    val aggregatesWithLimit = client.dataPoints
      .queryAggregatesById(
        54577852743225L,
        Instant.ofEpochMilli(0L),
        Instant.ofEpochMilli(1553795183461L),
    ...
    val aggregatesWithZeroLimit = client.dataPoints
      .queryAggregatesById(
        54577852743225L,
        Instant.ofEpochMilli(0L),
        Instant.ofEpochMilli(1553795183461L),
    ...
    val extAggregates = client.dataPoints
    ...
    (extAverages.map(_.timestamp).tail should contain)
      .theSameElementsInOrderAs(extStepInterpolation.map(_.timestamp))
    ...
    val aggregates2 = client.dataPoints
      .queryAggregatesById(
        54577852743225L,
        Instant.ofEpochMilli(0L),
        Instant.ofEpochMilli(1553795183461L),
    ...
    val extAggregatesWithLimit = client.dataPoints
      .queryAggregatesByExternalId(
        "VAL_23_FIC_92543_06:Z.X.Value",
        Instant.ofEpochMilli(0L),
        Instant.ofEpochMilli(1553795183461L),
    ...
    val extAggregatesWithZeroLimit = client.dataPoints
      .queryAggregatesByExternalId(
        "VAL_23_FIC_92543_06:Z.X.Value",
        Instant.ofEpochMilli(0L),
        Instant.ofEpochMilli(1553795183461L),
    ...
    (extSum2.map(_.timestamp).tail should contain)
      .theSameElementsInOrderAs(extStepInterpolation2.map(_.timestamp))
    aggregates2.keys should contain only ("sum", "stepInterpolation")
    assertThrows[CdpApiException] {
      val _ = client.dataPoints
        .queryAggregatesById(
          54577852743225L,
          Instant.ofEpochMilli(0L),
          Instant.ofEpochMilli(1553795183461L),
  ...
  private val sumsOnly = client.dataPoints
    .queryAggregatesById(
      54577852743225L,
      Instant.ofEpochMilli(0L),
      Instant.ofEpochMilli(1553795183461L),
