...
class LabelsTest extends SdkTestSpec with ReadBehaviours with WritableBehaviors with RetryWhile {
  ...
  def createRandItems(): Seq[LabelCreate] = {
    val dataSet: Seq[DataSet] = client.dataSets.retrieveByExternalIds(Seq(labelDataSet), ignoreUnknownIds = true).unsafeRunSync()
    val dataSetId = if (dataSet.isEmpty) {
      client.dataSets.createOne(DataSetCreate(name = Some(labelDataSet), externalId = Some(labelDataSet))).unsafeRunSync().id
    } else {
      dataSet.head.id
    }
    Seq(
  ...
  it should "support filter" in {
    ...
    val cleanupItems = randItems.flatMap(
      item => client.labels.filter(LabelsFilter(name = Some(item.name))).compile.toVector.unsafeRunSync()
    )
    client.labels.deleteByExternalIds(cleanupItems.map(_.externalId)).unsafeRunSync()

    client.labels.create(randItems).unsafeRunSync()

    val externalIdPrefix = randItems.head.externalId
    val byExternalId =
      client.labels.filter(LabelsFilter(externalIdPrefix = Some(externalIdPrefix))).compile.toVector.unsafeRunSync()
    ...
    val byName = client.labels.filter(LabelsFilter(name = Some(name))).compile.toVector.unsafeRunSync()
    byName should have length 1
    byName.head.externalId should be(randItems.last.externalId)

    val dataSetId = client.dataSets.retrieveByExternalId(labelDataSet).unsafeRunSync().id
    val byDataSetId = client.labels.filter(LabelsFilter(dataSetIds = Some(Seq(CogniteInternalId(dataSetId))))).compile.toVector.unsafeRunSync()
    byDataSetId should have length 1
    byDataSetId.head.externalId should be(randItems.last.externalId)

    val bydataSetExternalId = client.labels.filter(LabelsFilter(dataSetIds = Some(Seq(CogniteExternalId(labelDataSet))))).compile.toVector.unsafeRunSync()
