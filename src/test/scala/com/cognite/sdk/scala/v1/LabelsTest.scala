...
class LabelsTest extends SdkTestSpec with ReadBehaviours with WritableBehaviors with RetryWhile {
  ...
  def createRandItems(): Seq[LabelCreate] = {
    val dataSet: Seq[DataSet] = client.dataSets
      .retrieveByExternalIds(Seq(labelDataSet), ignoreUnknownIds = true)
      .unsafeRunSync()
    val dataSetId = if (dataSet.isEmpty) {
      client.dataSets
        .createOne(DataSetCreate(name = Some(labelDataSet), externalId = Some(labelDataSet)))
        .unsafeRunSync()
        .id
    } else {
      dataSet.head.id
  ...
  it should "support filter" in {
    ...
    val cleanupItems = randItems.flatMap(item =>
      client.labels.filter(LabelsFilter(name = Some(item.name))).compile.toVector.unsafeRunSync()
    )
    ...
    val byExternalId =
      client.labels
        .filter(LabelsFilter(externalIdPrefix = Some(externalIdPrefix)))
        .compile
        .toVector
        .unsafeRunSync()
    ...
    val byName =
      client.labels.filter(LabelsFilter(name = Some(name))).compile.toVector.unsafeRunSync()
    ...
    val byDataSetId = client.labels
      .filter(LabelsFilter(dataSetIds = Some(Seq(CogniteInternalId(dataSetId)))))
      .compile
      .toVector
      .unsafeRunSync()
    ...
    val bydataSetExternalId = client.labels
      .filter(LabelsFilter(dataSetIds = Some(Seq(CogniteExternalId(labelDataSet)))))
      .compile
      .toVector
      .unsafeRunSync()
