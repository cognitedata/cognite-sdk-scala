...
trait StringDataPointsResourceBehaviors extends Matchers with OptionValues with RetryWhile {
  this: AnyFlatSpec =>
  ...
  def stringDataPointsResource(
    ...
    it should "be possible to insert and delete string data points" in withStringTimeSeries {
      stringTimeSeries =>
        ...
        dataPoints
          .insertStrings(CogniteInternalId(stringTimeSeriesId), testStringDataPoints)
          .unsafeRunSync()
        ...
        retryWithExpectedResult[Option[StringDataPoint]](
          dataPoints
            .getLatestStringDataPoint(CogniteInternalId(stringTimeSeriesId))
            .unsafeRunSync(),
          dp => {
        ...
        val resultId2: Seq[_] = dataPoints
          .queryStrings(Seq(CogniteInternalId(stringTimeSeriesId)), start, end.plusMillis(1))
          .unsafeRunSync()
        resultId2.length shouldBe 1
        resultId2.headOption.value shouldBe resultId

        dataPoints
          .insertStrings(CogniteExternalId(stringTimeSeriesExternalId), testStringDataPoints)
          .unsafeRunSync()
        val resultExternalId: StringDataPointsByExternalIdResponse =
          retryWithExpectedResult[StringDataPointsByExternalIdResponse](
            dataPoints
              .queryStringsByExternalId(stringTimeSeriesExternalId, start, end.plusMillis(1))
              .unsafeRunSync(),
        ...
        val resultExternalId2: Seq[StringDataPointsByExternalIdResponse] = dataPoints
          .queryStringsByExternalIds(Seq(stringTimeSeriesExternalId), start, end.plusMillis(1))
          .unsafeRunSync()
        ...
        retryWithExpectedResult[Option[StringDataPoint]](
          dataPoints
            .getLatestStringDataPoint(CogniteExternalId(stringTimeSeriesExternalId))
            .unsafeRunSync(),
          l2 => {
    ...
    it should "be possible to insertStringsById and insertStringsByExternalId and delete string data points" in withStringTimeSeries {
      stringTimeSeries =>
        ...
        dataPoints
          .insertStringsByExternalId(stringTimeSeriesExternalId, testStringDataPoints)
          .unsafeRunSync()
        val resultExternalId: StringDataPointsByExternalIdResponse =
          retryWithExpectedResult[StringDataPointsByExternalIdResponse](
            dataPoints
    ...
    it should "support support query by externalId when ignoreUnknownIds=true" in {
      ...
      dataPoints
        .getLatestStringDataPoints(Seq(doesNotExist), ignoreUnknownIds = true)
        .unsafeRunSync() shouldBe empty
      dataPoints
        .getLatestDataPoints(Seq(doesNotExist), ignoreUnknownIds = true)
        .unsafeRunSync() shouldBe empty
      dataPoints
        .query(Seq(doesNotExist), Instant.EPOCH, Instant.now, ignoreUnknownIds = true)
        .unsafeRunSync() shouldBe empty
      dataPoints
        .queryStrings(Seq(doesNotExist), Instant.EPOCH, Instant.now, ignoreUnknownIds = true)
        .unsafeRunSync() shouldBe empty
    }
    ...
    it should "support support query by internal id when ignoreUnknownIds=true" in {
      ...
      dataPoints
        ...
        .unsafeRunSync() shouldBe empty
      dataPoints
        .queryByIds(Seq(doesNotExist.id), Instant.EPOCH, Instant.now, ignoreUnknownIds = true)
        .unsafeRunSync() shouldBe empty
      dataPoints
        .queryStrings(Seq(doesNotExist), Instant.EPOCH, Instant.now, ignoreUnknownIds = true)
        .unsafeRunSync() shouldBe empty
    ...
    it should "be an error insert or delete string data points for non-existing time series" in {
      ...
      val thrown = the[CdpApiException] thrownBy dataPoints
        .insertStrings(CogniteInternalId(unknownId), testStringDataPoints)
        .unsafeRunSync()
